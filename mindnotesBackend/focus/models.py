...
- FocusSession → FocusSessionMongo (real-time data)
- FocusPause → Embedded in FocusSessionMongo
- UserProgramDay → Stored in MongoDB
- FocusGoal → Can move to MongoDB

KEPT MODELS (Template/Reference Data):
- FocusProgram (program templates)
- ProgramDay (program structure)
- UserFocusProgram (enrollment tracking)
"""
...
class UserFocusProgram(Model):
    """
    MODIFIED: Keep enrollment in PostgreSQL, progress in MongoDB
    """
    ...
    # Progress tracking fields moved i
